...
const Comics = ({ addToCart }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">Comics</h1>
        ...
        <div className="flex-1">
          {filteredComics.length === 0 ? (
            <div className="bg-white p-6 rounded-md shadow text-center">
              <p className="text-gray-500">No se encontraron comics con los filtros seleccionados.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {filteredComics.map((comic) => (
                <ProductCard key={comic.id} product={comic} addToCart={addToCart} />
              ))}
            </div>
          )}
        </div>
